...
configPort_name = '/dev/tty.SLAB_USBtoUART3'
dataPort_name = '/dev/tty.SLAB_USBtoUART'
configFile_name = './chirp_3DPeople.cfg'
...
try:
	...
	MyWindow = pg.GraphicsLayoutWidget()
	...
	while(True):
		...
		if (sync == syncPattern):
			...
			# Clear the plot
			Plot_2D.clear()
			r1 = pg.QtWidgets.QGraphicsRectItem(boundaryBox[0], boundaryBox[2], -boundaryBox[0]+boundaryBox[1], -boundaryBox[2]+boundaryBox[3])
			r2 = pg.QtWidgets.QGraphicsRectItem(staticBoundaryBox[0], staticBoundaryBox[2], -staticBoundaryBox[0]+staticBoundaryBox[1], -staticBoundaryBox[2]+staticBoundaryBox[3])
